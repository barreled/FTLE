...
for xi in x:
  #print xi
  xd[xi] = i0
  xs[xi] = struct.pack('d', xi/1000) #convert from mm to m before writing to file
  i0 = i0 + 1
...
for yi in y: 
  #print yi
  yd[yi] = i0
  ys[yi] = struct.pack('d', yi/1000) #convert from mm to m before writing to file
  i0 = i0 + 1
...
for position1, index1, x1, y1, z1 in pos1:
  ...
  if (el - el0) > 4: print 'Element {:d} has {:d} nodes.'.format(position1, el - el0)
